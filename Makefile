...
CONFIG_OPTIONS := PROTOCOL PRE PARTY BITLENGTH FRACTIONAL FUNCTION_IDENTIFIER NUM_INPUTS \
				  DATTYPE PROCESS_NUM LIVE\
				  RANDOM_ALGORITHM USE_SSL_AES ARM USE_CUDA_GEMM \
				  SEND_BUFFER RECV_BUFFER VERIFY_BUFFER LOG_COMMUNICATION_ROUNDS \
				  USE_SSL CV_FIX\
				  A_KNOWN CONV_TRIPLES FC_TRIPLES BN2D_TRIPLES CHEETAH_THREADS CHEETAH_WAN_OPT CHEETAH_CONV_TYPE CHEETAH_DISCONNECT CHEETAH_BOOL_OT_TYPE ADDITIONAL_GEMM_THREADS ADDITIONAL_PPA_THREADS\
				  MODELOWNER DATAOWNER TRUNC_THEN_MULT TRUNC_APPROACH TRUNC_DELAYED VERIFY_CORRECTNESS COMPUTE_ARGMAX PUBLIC_WEIGHTS COMPRESS REDUCED_BITLENGTH_k REDUCED_BITLENGTH_m \
				...
				ROT_PREPROCESSING_OPT MODELWEIGHTS_KNOWN_DURING_PREPROCESSING MWK_PRESCRIBED_HE BEAVER_N_TUPLES RESHARE_OPT \
				RESHARE_OPT_SIM PPA4_MSB PPA_MSB RCA_MSB BIT_INJECTION_TRUNC_SIM CUT_FRACTIONAL_BITS_OPT A2B_CONV_BAKE \
