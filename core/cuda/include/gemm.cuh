...
cudaError_t CutlassGemmOp(int M, int N, int K, T alpha, T const* A, int lda, T const* B, int ldb, T beta, T* C, int ldc)
{

    CutlassGemm<T> gemm_operator;

    typename CutlassGemm<T>::Arguments args({M, N, K}, {A, lda}, {B, ldb}, {C, ldc}, {C, ldc}, {alpha, beta});
...
template <typename T>
void gemm(int M, int N, int K, T const* A, bool transpose_a, T const* B, bool transpose_b, T* C, bool transpose_c)
{
