...
class ABY2_ONLINE_Share
{
  ...
  public:
    ...
    ABY2_ONLINE_Share prepare_mult_public_fixed(const Datatype b,
                                                func_mul MULT,
                                                func_add ADD,
                                                func_sub SUB,
                                                func_trunc TRUNC,
...
/* c.l = getRandomVal(PSELF); */
...
#else // 4 mult, open could be shiftet to preprocessing
#if PARTY == 0
    ...
    template <typename func_add, typename func_sub, typename func_mul>
    ABY2_ONLINE_Share prepare_dot_and_assign(ABY2_ONLINE_Share b, Datatype assign, Datatype triple_c, func_add ADD, func_sub SUB, func_mul MULT) const
    {        
        Datatype lxly = triple_c;
        ABY2_ONLINE_Share c;
#if OPTIMIZED_COMPUTATION_MULT == 0 // 5 mult
...
/* c.l = getRandomVal(PSELF); */
    ...
    void complete_mult(func_add ADD, func_sub SUB)
    {
        Datatype msg = receive_from_live(PNEXT);
#if A2B_OPT == 1
        // Store msg as mx for S (P-1), lx will be zero
#endif
        m = ADD(m, msg);
    }
    template <typename func_add, typename func_sub, typename func_trunc>
    ...
    void prepare_bit2a(ABY2_ONLINE_Share out[])
    {
        ...
        for (int i = 0; i < BITLENGTH; i++)
        {
            ...
            out[i].m = SET_ALL_ZERO();
#endif
            out[i].m = OP_SUB(out[i].m,
                              OP_MULT(OP_SUB(OP_ADD(b0[i], b0[i]), PROMOTE(1)),
                                      OP_SUB(lb[i], OP_ADD(lxly, lxly))));  // mb -  (2mb - 1) * (lvi - 2 lxly)
            out[i].l = getRandomVal(PSELF);
            out[i].m = OP_ADD(out[i].m, out[i].l);
            send_to_live(PNEXT, out[i].m);
        }
    ...
    void complete_bit2a() { m = OP_ADD(m, receive_from_live(PNEXT)); }
    ...
    ABY2_ONLINE_Share prepare_dot3_and_assign(const ABY2_ONLINE_Share b,
                                   ...
                                   const Beaver3Tuple<Datatype>& beaver_triple,
                                   func_add ADD,
                                   func_sub SUB,
                                   func_mul MULT) const
    {
    ...
    template <typename func_add, typename func_sub, typename func_mul>
    ABY2_ONLINE_Share prepare_mult3(ABY2_ONLINE_Share b, ABY2_ONLINE_Share c, func_add ADD, func_sub SUB, func_mul MULT)
        const
    {
    ...
    template <typename func_add, typename func_sub, typename func_mul>
    ABY2_ONLINE_Share prepare_mult3_and_assign(ABY2_ONLINE_Share b, ABY2_ONLINE_Share c, const Datatype assign, const Beaver3Tuple<Datatype>& beaver_triple, func_add ADD, func_sub SUB, func_mul MULT)
        const
    {
    ...
    template <typename func_add, typename func_sub, typename func_mul>
    ABY2_ONLINE_Share prepare_mult4(ABY2_ONLINE_Share b,
                                    ABY2_ONLINE_Share c,
                                    ABY2_ONLINE_Share d,
                                    func_add ADD,
                                    func_sub SUB,
    ...
    static ABY2_ONLINE_Share public_val(Datatype a) { return ABY2_ONLINE_Share(a, SET_ALL_ZERO()); }

    ABY2_ONLINE_Share Not() const { return ABY2_ONLINE_Share(NOT(m), l); }
    ...
    template <typename func_mul, typename func_add, typename func_sub, typename func_trunc>
    ABY2_ONLINE_Share prepare_trunc_share(func_mul MULT,
                                          func_add ADD,
                                          func_sub SUB,
                                          func_trunc TRUNC,
                                          int fractional_bits = FRACTIONAL) const
...
auto tempml = OP_SUB(X[i].m, X[i].l);
    ...
    static void
    GEMM(ABY2_ONLINE_Share* a, ABY2_ONLINE_Share* b, ABY2_ONLINE_Share* c, int m, int n, int k, bool a_fixed = false)
    {
        const int factor = DATTYPE / BITLENGTH;
