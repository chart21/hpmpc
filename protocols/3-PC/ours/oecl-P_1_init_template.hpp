...
/*     receiving_args[t].elements_to_rec[rounds-1]+=1; */
    ...
    template <typename func_mul>
    OECL1_init mult_a_known_to_evaluators(const OECL1_init b,
                                                func_mul MULT) const
    {
        return OECL1_init();
    }
    ...
    template <typename func_mul, typename func_add, typename func_sub, typename func_trunc>
        OECL1_init local_mult_and_trunc(const Datatype b,
                                        func_mul MULT,
                                        func_add ADD,
                                        func_sub SUB,
                                        func_trunc TRUNC,
    ...
    void prepare_opt_bit_injection_with_trunc(OECL1_init x[], OECL1_init out[], Datatype trunc_factor, int fractional_bits = FRACTIONAL)
    {
