...
class OECL2_Share
{
  ...
  public:
    ...
    OECL2_Share mult_a_known_to_evaluators(const OECL2_Share b,
                                                func_mul MULT) const
    {
        return OECL2_Share(MULT(p1, b.p1), MULT(p1, b.p2)); // mb_i (a) = ab + lb_i * a
    }
    ...
    template <typename func_add, typename func_sub>
        void prepare_remask(func_add ADD, func_sub SUB)
        {
            ...
            send_to_live(P_1, SUB(new_l, p2)); // replace old mask with new mask
            p2 = new_l; // lvi' = lni
        }
    ...
    template <typename func_mul, typename func_add, typename func_sub, typename func_trunc>
        OECL2_Share local_mult_and_trunc(const Datatype b,
                                        func_mul MULT,
                                        func_add ADD,
                                        func_sub SUB,
                                        func_trunc TRUNC,
                                        int fractional_bits = FRACTIONAL) const
        {
            return OECL2_Share(TRUNC(MULT(p1, b), fractional_bits), p2);
        }
    ...
    void prepare_opt_bit_injection_with_trunc(OECL2_Share a[], OECL2_Share out[], Datatype trunc_factor, int fractional_bits = FRACTIONAL)
    {
        Datatype b0[BITLENGTH]{0};
        b0[BITLENGTH - 1] = FUNC_XOR(p1, p2);  // convert b to an arithemtic value
        alignas(sizeof(Datatype)) UINT_TYPE temp2[DATTYPE];
        ...
        for (int i = 0; i < BITLENGTH; i++)
        {
            ...
            m1 = OP_MULT(m1, OP_SUB(m01, OP_MULT(a0, m00)));
            m1 = OP_SUB(m1, OP_MULT(b0[i], a[i].p2));
            m1 = OP_TRUNCF(OP_MULT(m1, trunc_factor), fractional_bits);
            send_to_live(P_1, OP_ADD(m1, z2));
            out[i].p1 = m1;
            out[i].p2 = z2;
        }
    ...
    template <typename func_add, typename func_sub, typename func_mul>
    void prepare_Conv_BN_Accum(const OECL2_Share x, Datatype* result, func_add ADD, func_sub SUB, func_mul MULT) const
    {
        result[0] = ADD(result[0], ADD(p1, p2)); //  sum(mw)
        result[1] = ADD(result[1], p2); // sum(lw_i)
        result[2] = ADD(result[2], ADD(x.p1, x.p2)); //  sum(mx)
        result[3] = ADD(result[3], x.p2); // sum(lx_i)
        result[4] = ADD(result[4], MULT(ADD(p1, p2), ADD(x.p1, x.p2))); // sum(mw*mx)                                                     
        result[5] = ADD(result[5], MULT(ADD(x.p1, x.p2), p2)); // sum(mx*lw_i) 
        result[6] = ADD(result[6], MULT(ADD(p1, p2), x.p2)); // sum(mw*lx_i) 
    }
    
    template <typename func_add, typename func_sub, typename func_mul, typename func_trunc>
    void calculate_conv_bn(const OECL2_Share mu, const OECL2_Share sigma, const Datatype* accum, func_add ADD, func_sub SUB, func_mul MULT, func_trunc TRUNC)
    {
        ...
        const auto msigma = ADD(sigma.p1, sigma.p2);
        const auto lsigma_i = sigma.p2;
#if PRE == 1
        const auto lx_lw = pre_receive_from_live(P_0);
        const auto lw_lsigma = pre_receive_from_live(P_0);
        const auto lx_lsigma = pre_receive_from_live(P_0);
#else
        const auto lx_lw = receive_from_live(P_0);
        const auto lw_lsigma = receive_from_live(P_0);
        const auto lx_lsigma = receive_from_live(P_0);
#endif
        ...
        OECL2_Share three_terms = OECL2_Share(ADD(msigma_terms, remaining_terms));
        OECL2_Share sigma_mu = mu.prepare_dot(sigma, ADD, SUB, MULT);
