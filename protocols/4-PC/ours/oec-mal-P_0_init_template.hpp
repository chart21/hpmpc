...
void prepare_trunc_2k_inputs(func_add ADD, func_sub SUB, func_xor XOR, func_and AND, func_trunc trunc, OEC_MAL0_init& r_mk2, OEC_MAL0_init& r_msb, OEC_MAL0_init& c, OEC_MAL0_init& c_prime, int fractional_bits = FRACTIONAL) const{
#if PROTOCOL == 12 || PROTOCOL == 8 || PRE == 1
    store_compare_view_init(P_2);
    store_compare_view_init(P_2);
#else
...
OEC_MAL0_init prepare_trunc_exact_xmod2t(func_add ADD, func_sub SUB, func_xor XOR, func_and AND, int fractional_bits = FRACTIONAL) const {
    return OEC_MAL0_init();
}
...
OEC_MAL0_init prepare_mult_public_fixed(const Datatype b, func_mul MULT, func_add ADD, func_sub SUB, func_trunc TRUNC, int fractional_bits = FRACTIONAL) const
{
#if PROTOCOL == 12 || PROTOCOL == 8 || PRE == 1
    store_compare_view_init(P_2);
#else
...
template <typename func_mul, typename func_add, typename func_sub, typename func_trunc>
OEC_MAL0_init prepare_trunc_share(func_mul MULT, func_add ADD, func_sub SUB, func_trunc TRUNC, int fractional_bits = FRACTIONAL) const
{
#if PROTOCOL == 12 || PROTOCOL == 8 || PRE == 1
    store_compare_view_init(P_2);
#else
...
static void CONV_2D(const OEC_MAL0_init* X, const OEC_MAL0_init* W, OEC_MAL0_init* Y, int batchSize, int inh, int inw, int din, int dout, int wh, int ww, int padding, int stride, int dilation = 1){
}
#endif
