...
def create_and_save_plot(data, bitlength, trunc_then_mult, trunc_delayed, file_prefix):
    num_entries = len(data['TRUNC_APPROACH'].unique())
    legend_ncol = num_entries
    base_width = 3.08
    extra_width_per_entry = 1.0  # Adjust spacing as needed
    fig_width = max(base_width, 2.5 + extra_width_per_entry * num_entries)

    if full:
        fig, ax = plt.subplots(figsize=(fig_width, 3.08))
    else:
        fig, ax = plt.subplots(figsize=(fig_width, 3.08))

    colors = plt.cm.rainbow(np.linspace(0, 1, num_entries))
    legend_elements = []
    ...
    ax.legend(
        ...
        loc='lower center',
        bbox_to_anchor=(0.5, 1.12),  # Push the legend higher above the plot
        ncol=legend_ncol,
        frameon=False,
        handlelength=1.5,
        handletextpad=0.5,
    ...
    plt.tight_layout(rect=[0, 0, 1, 0.88])  # Reserve space at the top for the legend
