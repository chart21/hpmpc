...
class Additive_Share : public Share_Type {
  public:
    ...
    Additive_Share(UINT_TYPE a) : Share_Type() { *this = Share_Type::public_val(PROMOTE(a)); }

    Additive_Share operator+(const Additive_Share<Datatype, Share_Type>& b) const {
        return Additive_Share(Share_Type::Add(b, OP_ADD));
    }

    void operator+=(const Additive_Share<Datatype, Share_Type>& b) { *this = *this + b; }
    ...
    Additive_Share operator*(const Additive_Share<Datatype, Share_Type>& b) const {
        /* return Additive_Share(Share_Type::prepare_dot(b, OP_ADD, OP_SUB,
         * OP_MULT)); */
        return Additive_Share(Share_Type::prepare_mult(b, OP_ADD, OP_SUB, OP_MULT));
    }

    Additive_Share operator*(const UINT_TYPE other) const {
        return Additive_Share(Share_Type::prepare_mult_public_fixed(PROMOTE(other), OP_MULT, OP_ADD,
                                                                    OP_SUB, FUNC_TRUNC));
    }

    void operator*=(const UINT_TYPE other) { prepare_mult_public_fixed(other); }

    void operator*=(const Additive_Share<Datatype, Share_Type>& other) { *this = *this * other; }
    ...
    void prepare_mult_public_fixed(const UINT_TYPE b) {
        *this =
            Share_Type::prepare_mult_public_fixed(PROMOTE(b), OP_MULT, OP_ADD, OP_SUB, FUNC_TRUNC);
    }

    void complete_public_mult_fixed() { Share_Type::complete_public_mult_fixed(OP_ADD, OP_SUB); }

    bool operator==(const Additive_Share& b [[maybe_unused]]) const {
    ...
    //       if (current_phase != PHASE_INIT) { //TODO: Should only happen
    //       either in PRE or in live pahse
    ...
    void prepare_reveal_to_all() const { Share_Type::prepare_reveal_to_all(); }
    // void prepare_trunc_2k_inputs(Share_Type& rmk2, Share_Type& rmsb,
    // Share_Type& c, Share_Type& c_prime)
    // {
    //    Share_Type::prepare_trunc_2k_inputs(OP_ADD, OP_SUB, FUNC_XOR,
    //    FUNC_AND, FUNC_TRUNC, rmk2, rmsb, c, c_prime);
    // }

    // void complete_trunc_2k_inputs(Share_Type& rmk2, Share_Type& rmsb,
    // Share_Type& c, Share_Type& c_prime)
    // {
    //     Share_Type::complete_trunc_2k_inputs(OP_ADD, OP_SUB, FUNC_XOR,
    //     FUNC_AND, FUNC_TRUNC, rmk2, rmsb, c, c_prime);
    // }

    /* Additive_Share prepare_trunc_2k() const */
    /* { */
    /*     return Additive_Share(Share_Type::prepare_trunc_2k(OP_ADD, OP_SUB,
     * FUNC_XOR, FUNC_AND, FUNC_TRUNC)); */
    /* } */
    ...
    UINT_TYPE complete_reveal_to_all_single() const {
        auto res =
            Share_Type::complete_Reveal(OP_ADD, OP_SUB); // TODO: Use extract method from Intrinsics
    ...
    Additive_Share prepare_mult3(const Additive_Share<Datatype, Share_Type>& b,
                                 const Additive_Share<Datatype, Share_Type>& c) const {
    ...
    void complete_mult3() { Share_Type::complete_mult3(OP_ADD, OP_SUB); }

    Additive_Share prepare_mult4(const Additive_Share<Datatype, Share_Type>& b,
                                 const Additive_Share<Datatype, Share_Type>& c,
                                 const Additive_Share<Datatype, Share_Type>& d) const {
        return Additive_Share(Share_Type::prepare_mult4(b, c, d, OP_ADD, OP_SUB, OP_MULT));
    }

    void complete_mult4() { Share_Type::complete_mult4(OP_ADD, OP_SUB); }
    ...
    Additive_Share prepare_dot3(const Additive_Share<Datatype, Share_Type>& b,
                                const Additive_Share<Datatype, Share_Type>& c) const {
        return Additive_Share(Share_Type::prepare_dot3(b, c, OP_ADD, OP_SUB, OP_MULT));
    }

    Additive_Share prepare_dot4(const Additive_Share<Datatype, Share_Type>& b,
                                const Additive_Share<Datatype, Share_Type>& c,
                                const Additive_Share<Datatype, Share_Type>& d) const {
...
#else
#if PROTOCOL == 2
        Share_Type::mask_and_send_dot(OP_SUB, OP_MULT); // Replicated needs custom overloads because
                                                        // division by 3 is required
#else
