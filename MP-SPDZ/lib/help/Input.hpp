...
class Input {
  public:
    explicit Input(const std::string& path, const int& player_num)
        : player_num(player_num), cur(0) {
...
std::vector<Input>::iterator get_input_from(const int& player_num) {
    auto it = inputs.begin();
    for (; it != inputs.end(); ++it) {
        if (it->get_player() == player_num)
            return it;
    }
