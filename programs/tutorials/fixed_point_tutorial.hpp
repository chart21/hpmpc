...
void Fixed_Point_Tutorial(DATATYPE *res) 
{
    ...
    fixed_point_secret.template prepare_receive_from<P_0>(PROMOTE(fixed_point_value)); // After conversion, fixed point values can ban be treated as integers
    Share::communicate();
    ...
    UINT_TYPE public_fixed_point_value = FloatFixedConverter<float, INT_TYPE, UINT_TYPE, FRACTIONAL>::float_to_ufixed(2.5f); 
    result = result.prepare_mult_public_fixed(public_fixed_point_value); // While public integer values can be multiplied without communication, multiplying a secret with a public fixed point values requires communication
    Share::communicate();
    result.complete_public_mult_fixed(); // Complete multiplication with a public fixed point value
        //Division with public values
    UINT_TYPE reciprocal = FloatFixedConverter<float, INT_TYPE, UINT_TYPE, FRACTIONAL>::float_to_ufixed(1/2.5f); //simply invert the fixed point value and multiply
    ...
    for(int i = 0; i < vectorization_factor; i++)
        converted_output[i] = FloatFixedConverter<INT_TYPE, UINT_TYPE, float, FRACTIONAL>::ufixed_to_float(output_vectorized[i]); // Convert the fixed point value back to a floating point value
