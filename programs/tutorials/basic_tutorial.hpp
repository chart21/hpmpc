...
#define RESULTTYPE DATATYPE  // Each main function should define this to make a result accessible after the MPC protocol has finished
...
void Basic_Tutorial(DATATYPE *res) // The main entry pint needs to define a result pointer, DATATYPE is specified in config.h
{
    ...
    UINT_TYPE integer_value = 5; // Plaintext integer with the same BITLENGTH as secret shares, BITLENGTH is defined in config.h
    UINT_TYPE fixed_point_value = FloatFixedConverter<float, INT_TYPE, UINT_TYPE, FRACTIONAL>::float_to_ufixed(3.5f); // Plaintext fixed point value with FRACTIONAL bits of precision, defined in config.h


    //---------------------------------//
    //The vectorized programming model
    const int vectorization_factor = DATTYPE/BITLENGTH; // Each instruction in the framework is inherently vectorized.
    ...
    orthogonalize_arithmetic(input_of_party_0, &vectorized_input_of_party_0,1); // A set of inputs gets vectorized in a single variable depending on the chosen registerlength
    ...
    orthogonalize_arithmetic(vectorized_public_input, &public_input,1); // A set of inputs gets vectorized in a single variable depending on the chosen registerlength
    public_share = A::get_share_from_public_dat(public_input); // A public value is shared among all parties
   

        // Identical private inputs
    A single_secret;
    single_secret.template prepare_receive_from<P_1>(PROMOTE(9)); // Party 1 prepares the secret share of the value 9, PROMOTE ensures all vectorized values are set to the same single value
    Share::communicate(); 
    single_secret.template complete_receive_from<P_1>(); // The secret is now shared among all parties

        // Identical public inputs
    A another_public_share = 2; // A public value is shared among all parties, all vectorized values are set to the same single value
    ...
    Share::communicate(); // Communication needs to be triggered explicitly
    result.complete_mult_without_trunc(); // Complete multiplication, truncation is not required for integer multiplication
    ...
    for(int i = 0; i < len; i++) 
    {
        array_a[i].template complete_receive_from<P_0>();  // Make sure the order of the completions matches the order of the prepare calls
        array_b[i].template complete_receive_from<P_1>(); 
    
    
    }
    for(int i = 0; i < len; i++) 
        arrary_c[i] = array_a[i].prepare_mult(array_b[i]); // Only after complete operations are done, shares should be accessed
    Share::communicate(); 
    for(int i = 0; i < len; i++) 
        arrary_c[i].complete_mult_without_trunc(); // Only after complete operations are done, shares should be accessed
    ...
    alignas(sizeof(DATATYPE)) UINT_TYPE result_array_vectorized[len][vectorization_factor]; // the result is still vectorized
    ...
    for(int i = 0; i < len; i++)
        for(int j = 0; j < vectorization_factor; j++)
            print_online("Output " + std::to_string(i) + "," + std::to_string(j) + ": " + std::to_string(result_array_vectorized[i][j]) + "\n"); // Print the result in the online phase
}
